...
import pandas as pd
from functions import UsersRecommend, sentiment_analysis, UsersWorstDeveloper, PlayTimeGenre, UserForGenre, recomendacion_por_usuario, recomendacion_por_juego

# Crea una instancia de la aplicación FastAPI
app = FastAPI()
# http://127.0.0.1:8000/ Esta es la ruta local de acceso a la fastapi
@app.get("/")
def index():
...
async def endpoint1(genero: str):
    """
    Descripción: Esta función retorna el año con más horas jugadas
    
    Parámetros:
        - genero (str): Se recibe el genero para el cuál se consultará el año con mayor horas jugadas
...
@app.get("/recomendacion_Juegos/{item_id}", tags=['recomendacion por juego'])
async def item(item_id: int):
    ...
    try:
        item_id = int(item_id) 
        resultado= recomendacion_por_juego(item_id)
        return resultado
    except Exception as e:
        return {"error":str(e)}

@app.get("/recomendacion_Usuarios/{user_id}", tags=['recomendacion por usuario'])
async def item(id_usuario: int):
    """
    Descripción: Ingresando el id del usuario, devuelve una lista con 5 juegos recomendados para ese usuario
    
    Parámetros:
        - id_usuario (str): Id del producto para el cual se busca la recomendación. Debe ser un número, ejemplo: 761140
    ...
    try:
        ...
        resultado= recomendacion_por_usuario(id_usuario)
        return resultado
    except Exception as e:
        return {"error":str(e)}